    ...
    >
      <CarouselContent className="h-[360px]">
        {photos.length > 0 ? (
          ...
          // Show placeholder slots when nothing is uploaded yet
          Array.from({ length: 3 }).map((_, index) => (
            <CarouselItem key={index} className="basis-1/3">
              <div className="h-full p-2">
                <div className="w-full h-full rounded-xl bg-[var(--color-border-gray)] flex items-center justify-center text-sm text-[var(--color-muted)]">
                  Photo
                </div>
              </div>
            </CarouselItem>
