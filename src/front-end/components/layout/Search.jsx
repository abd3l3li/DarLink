...
const filterOptions = [
	...
	{ name: "price", label: "Price Range", placeholder: "Select range", options: ["0 - 2000 DH", "2000 - 4000 DH", "4000+ DH"] },
];
...
export default function FloatingSearchBar() {
	...
	return (
		<div className="mt-16 w-full max-w-5xl rounded-2xl bg-[var(--color-surface)] p-6 shadow-2xl">
			<div className="grid grid-cols-1 gap-4 md:grid-cols-4 lg:gap-6">
				{filterOptions.map(({ name, label, placeholder, options }) => (
					<div key={name} className="flex flex-col text-left">
						...
						<select
							...
							className="w-full rounded-xl bg-[var(--color-bg)] px-4 py-4 text-base font-medium text-[var(--color-text)] focus:ring-2 focus:ring-[var(--color-primary)]"
						>
				...
				<div className="flex items-end">
					<button
						...
						className="flex w-full items-center justify-center gap-2 rounded-xl bg-[var(--color-secondary)] px-8 py-4 text-base font-bold text-[var(--color-surface)] shadow-lg shadow-[var(--color-secondary)]/30 transition-all hover:scale-[1.02] active:scale-95"
					>
